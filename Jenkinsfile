...
properties([
    ...
    parameters([
        ...
        choice(name: 'SAUCELABS_BROWSERS', defaultValue: '', choices: ['firefox', 'chrome', 'safari', 'edge'], description: 'Which browser envs do you want to run on saucelabs', )
    ]),
    pipelineTriggers([])
])

echo "Browsers: ${SAUCELABS_BROWSERS}"
return;
...
node ('docker') {
    ...
    try {
        ...
        //
        athImg.inside("-v ${env.HOME}/.m2:/home/bouser/.m2" /*"--net=container:blueo-selenium"*/) {
            try {
                ...
                sauce('saucelabs') {
                    sauceconnect(useGeneratedTunnelIdentifier: true, verboseLogging: true) {
                        sh "./run.sh -a=./blueocean-plugin/blueocean/ --no-selenium --env=firefox,chrome,safari,edge"
                    }
                }
            } catch (err) {
